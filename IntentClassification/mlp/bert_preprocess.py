...
def create_mlp_full_dataset(input_file_name, out_file_name):
    out_file = open(out_file_name, WRITE)
    raw_train_data = get_data(input_file_name)
    out_file.write(
        'desc' + '\t' + 'agent_pos_x' + '\t' + 'agent_pos_y' + '\t' +
        'agent_pos_z' + '\t' + 'dist_to_obj' + '\t' +
        'dist_to_recep' + '\t' + 'obj_relevant' +
        '\t' + 'recep_relevant' + '\t' + 'intent' + '\n')
...
def get_visual_information(scene_desc):
    ...
    return [current_agent_pos_x, current_agent_pos_y, current_agent_pos_z,
            dist_to_obj, dist_to_recep, obj_relevant, recep_relevant]
